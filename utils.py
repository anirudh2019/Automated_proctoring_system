...
def print_fps(frame, pTime):
    ...
    fps = 1/(cTime - pTime[0])
    pTime[0] = cTime
    cv2.putText(frame, f"FPS : {int(fps)}", (15,30),cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255,0,0),2)
    return frame
...
def print_faces(frame, faces):
    for face in faces:
        ...
        if len(faces) == 1:
            ...
            cv2.putText(frame, "Mouth is Close!", (15,55), font, 0.5, (255,0,0),2) if face.mouth.status=="close" else cv2.putText(frame, "Mouth is Open!", (15,55), font, 0.5, (255,0,0),2)
    return frame
